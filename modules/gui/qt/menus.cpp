...
#include "components/player_controler.hpp"                      /* Input Management */
...
QMenu *VLCMenuBar::FileMenu( intf_thread_t *p_intf, QWidget *parent, MainInterface *mi )
{
    ...
    action = addMPLStaticEntry( p_intf, menu, qtr( "Quit at the end of playlist" ), "",
                               &PlaylistControlerModel::playAndExitChanged );
...
static inline void VolumeEntries( intf_thread_t *p_intf, QMenu *current )
{
    ...
    current->addAction( QIcon( ":/toolbar/volume-high.svg" ), qtr( "&Increase Volume" ), THEMIM, &PlayerControler::setVolumeUp );
    current->addAction( QIcon( ":/toolbar/volume-low.svg" ), qtr( "D&ecrease Volume" ), THEMIM, &PlayerControler::setVolumeDown );
    current->addAction( QIcon( ":/toolbar/volume-muted.svg" ), qtr( "&Mute" ), THEMIM, &PlayerControler::toggleMuted );
}
...
QMenu *VLCMenuBar::VideoMenu( intf_thread_t *p_intf, QMenu *current )
{
    if( current->isEmpty() )
    {
        ...
        connect(THEMIM, &PlayerControler::hasVideoOutputChanged, action, &QAction::setEnabled);
        ...
        connect(snapshotAction, &QAction::triggered, THEMIM, &PlayerControler::snapshot);
...
#define ADD_ACTION( title, slot, visibleSignal, visible ) \
    ...
    ADD_ACTION( qtr("Previous Title"), &PlayerControler::titlePrev, &PlayerControler::hasTitlesChanged, THEMIM->hasTitles() );
    ADD_ACTION( qtr("Next Title"), &PlayerControler::titleNext, &PlayerControler::hasTitlesChanged, THEMIM->hasTitles() );
    ADD_ACTION( qtr("Previous Chapter"), &PlayerControler::chapterPrev, &PlayerControler::hasChaptersChanged, THEMIM->hasChapters() );
    ADD_ACTION( qtr("Next Chapter"), &PlayerControler::chapterNext, &PlayerControler::hasChaptersChanged, THEMIM->hasChapters() );
...
void VLCMenuBar::PopupMenuPlaylistEntries( QMenu *menu, intf_thread_t *p_intf )
{
    ...
    if( !hasInput || THEMIM->getPlayingState() != PlayerControler::PLAYING_STATE_PLAYING )
    {
    ...
    else
    {
        action = addMPLStaticEntry( p_intf, menu, qtr( "Pause" ),
                ":/toolbar/pause_b.svg", &PlaylistControlerModel::togglePlayPause );
    }

    /* Stop */
    action = addMPLStaticEntry( p_intf, menu, qtr( "&Stop" ),
            ":/toolbar/stop_b.svg", &PlaylistControlerModel::stop );
    if( !hasInput )
    ...
    QAction* previousAction = addMPLStaticEntry( p_intf, menu, qtr( "Pre&vious" ),
            ":/toolbar/previous_b.svg", &PlaylistControlerModel::prev );
    previousAction->setEnabled( !bPlaylistEmpty );
    connect( THEMPL, &PlaylistControlerModel::isEmptyChanged, previousAction, &QAction::setDisabled);

    QAction* nextAction = addMPLStaticEntry( p_intf, menu, qtr( "Ne&xt" ),
            ":/toolbar/next_b.svg", &PlaylistControlerModel::next );
    nextAction->setEnabled( !bPlaylistEmpty );
    connect( THEMPL, &PlaylistControlerModel::isEmptyChanged, nextAction, &QAction::setDisabled);

    action = menu->addAction( qtr( "Record" ), THEMIM, &PlayerControler::toggleRecord );
...
QMenu* VLCMenuBar::PopupMenu( intf_thread_t *p_intf, bool show )
{
    ...
    {
        QMenu *submenu;
        PlayerControler::VoutPtr p_vout = THEMIM->getVout();
...
void VLCMenuBar::updateAudioDevice( intf_thread_t * p_intf, QMenu *current )
{
    ...
    current->clear();
    PlayerControler::AoutPtr aout = THEMIM->getAout();
    if (!aout)
        return;
